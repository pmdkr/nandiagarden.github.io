...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            ...
            note = 'Thanks for ordering ! Your %s %s and %s pizza on the way!' %(filled_form.cleaned_data['size'],
            filled_form.cleaned_data['topping1'],
            filled_form.cleaned_data['topping2'],)
            filled_form = PizzaForm()
        else:
            created_pizza_pk=None
            note='Pizza order has been failed,try  again.'
...
def pizzas(request):
    ...
    if request.method=='POST':
        ...
        if filled_formset.is_valid():
            ...
            note='ordered is not created, please try again'
